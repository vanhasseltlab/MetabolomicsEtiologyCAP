## Supplement code data analysis: elastic net with covariates
...
#   - Elastic net with covariates sex and age for comparison of 
...
## Data analysis: Elastic net regression (without covariates)-------------------------------------
...
for (s in 1:100){    
  ...
  for (i in list(spneu_rest, atyp_rest, vir_bac)){
    ...
    for (f in 1:5){
      ...
      for (f2 in (1:5)[(1:5)!=f]){
        ...
        for (a in alphas){
          for (l in labdas){
            ...
            trainfold2_covariates <- cbind(as.matrix(trainfold2$leeftijd), as.matrix(as.numeric(trainfold2$geslacht)), as.matrix(trainfold2[, metrange]))
            colnames(trainfold2_covariates)[1] <- "age"
            colnames(trainfold2_covariates)[2] <- "sex"
            
            elnet2 <- glmnet(x= trainfold2_covariates, 
                             y= as.matrix(trainfold2[, "test_groups"]), 
                             family = "binomial",alpha = a, lambda = l, 
                             penalty.factor = c(rep(0, 2), rep(1, ncol(as.matrix(trainfold2[, metrange])))))
            
            testfold2_covariates <- cbind(as.matrix(testfold2$leeftijd), as.matrix(as.numeric(testfold2$geslacht)), as.matrix(testfold2[, metrange]))
            colnames(testfold2_covariates)[1] <- "age"
            colnames(testfold2_covariates)[2] <- "sex"
            p2 <- predict(elnet2, testfold2_covariates, type = "response")
      ...
      trainfold_covariates <- cbind(as.matrix(trainfold$leeftijd), as.matrix(as.numeric(trainfold$geslacht)), as.matrix(trainfold[, metrange]))
      colnames(trainfold_covariates)[1] <- "age"
      colnames(trainfold_covariates)[2] <- "sex"
      
      elnet <- glmnet(x= trainfold_covariates, 
                      y= as.matrix(trainfold[, "test_groups"]), 
                      family = "binomial",alpha = a_opt, lambda = l_opt, 
                      penalty.factor = c(rep(0, 2), rep(1, ncol(as.matrix(trainfold[, metrange])))))
      
      # Predict testfold with chosen alpha and lambda
      testfold_covariates <- cbind(as.matrix(testfold$leeftijd), as.matrix(as.numeric(testfold$geslacht)), as.matrix(testfold[, metrange]))
      colnames(testfold_covariates)[1] <- "age"
      colnames(testfold_covariates)[2] <- "sex"
      p <- predict(elnet, testfold_covariates, type = "response")
...
save(repeat_data, file = paste(datafolder, "Results/", "04_Elastic_net_with_covariates_results", sep=""))
