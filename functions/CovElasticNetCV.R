CovElasticNetCV <- function(dat, metrange, cov, outcome, alphas, lambdas, repeats, folds){ 
      ...
      for (f2 in (1:folds)[(1:folds) != f]) {
        ...
        for (a in alphas){
          for (l in lambdas){
            ...
            elnet2 <- glmnet(x = as.matrix(trainfold2[, variables]), 
                             y = as.matrix(trainfold2[, outcome]), 
                             family = "binomial", alpha = a, lambda = l, 
                             penalty.factor = c(rep(1, length(metrange)), 
                                                rep(0, length(cov))))
            
            p2 <- predict(elnet2, as.matrix(testfold2[, variables]), type = "response")
